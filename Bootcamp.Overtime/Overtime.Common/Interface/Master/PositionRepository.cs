...
namespace Overtime.Common.Interface.Master
{
    public class PositionRepository : iPositionRepository
    {
        ...
        public List<Position> Search(string search, string cmb)
        {
            if(cmb == "Name")
            {
                var searchName = _context.Position.Where(x => x.isDelete == false && x.Id.ToString().Contains(search)).ToList();
                return searchName;
            }
            else if(cmb == "Id")
            {
                var searchId = _context.Position.Where(x => x.isDelete == false && x.name.ToString().Contains(search)).ToList();
                return searchId;
            }
            else
            {
                var refresh = _context.Position.Where(x => x.isDelete == false ).ToList();
                return refresh;
            }
            //throw new NotImplementedException();
        }
